document.addEventListener("DOMContentLoaded", () => {
    ...
    if (typeof config === 'undefined') { console.error("ERREUR : config.js n'est pas chargé."); return; }
    ...
    const avatarEl = document.getElementById("profile-avatar"); if(avatarEl) avatarEl.src = config.profile.avatar;
    const faviconEl = document.getElementById("favicon-link"); if(faviconEl && config.profile.favicon) faviconEl.href = config.profile.favicon;
    const nameEl = document.getElementById("profile-name"); if(nameEl) nameEl.innerText = config.profile.name;
    const statusEl = document.getElementById("profile-status"); if(statusEl) statusEl.innerText = config.profile.status;
    const bioEl = document.getElementById("profile-bio"); if(bioEl) bioEl.innerText = config.profile.bio;
    const footerEl = document.getElementById("footer-copy"); if(footerEl) footerEl.innerHTML = `&copy; ${new Date().getFullYear()} ${escapeHTML(config.profile.name)}.`;
    const ghBtn = document.getElementById("link-github"); if(ghBtn && config.social.github) ghBtn.href = config.social.github;
    const lkBtn = document.getElementById("link-linkedin"); if(lkBtn && config.social.linkedin) lkBtn.href = config.social.linkedin;
    ...
    function setupModal(triggerId, modalId, closeBtnId) {
        ...
        if (trigger && modal) { trigger.addEventListener("click", (e) => { e.preventDefault(); modal.style.display = "flex"; }); }
        ...
        emailTrigger.addEventListener("click", () => {
            ...
            if (window.turnstile) {
                if (widgetId !== null) turnstile.reset(widgetId);
                    ...
                    } catch (e) { if(captchaContainer) captchaContainer.innerHTML = "Erreur sécu."; }
                }
            }
        });
        if(copyBtn) {
    ...
    // --- 7. PROJETS (AVEC EXPAND ARROW + AUTO DATE) ---
    ...
    if (grid && config.projects) {
        config.projects.forEach((proj, index) => {
            ...
            // Construction HTML avec la flèche "expand-icon"
            div.innerHTML = `
            ...
            if (config.profile.githubUser && config.profile.githubRepo && proj.path) {
                ...
                fetch(apiUrl).then(res => { if (!res.ok) throw new Error("API Error"); return res.json(); })
                    .then(data => {
                        if (data && data.length > 0) {
                            ...
                            if (badgeEl && diffDays <= 30) { badgeEl.innerHTML = `<span class="new-badge">Nouveau</span>`; }
                        }
            ...
            // CLIC HEADER (Ouvre le PDF + Tourne la flèche)
            const headerDiv = div.querySelector('.card-header');
            headerDiv.addEventListener("click", () => { 
                div.classList.toggle('active'); // Pour la rotation CSS
                togglePDF(vid, fullPdfUrl); 
            });

            // CLIC INFO (Ouvre la modale)
    ...
    // --- 8. PARCOURS & 9. COMPETENCES & 10. CERTIFICATIONS ---
    const expList = document.getElementById("exp-list");
    const EXP_LIMIT = 5;
    if(expList && config.experiences) {
        config.experiences.forEach((exp, index) => {
            const li = document.createElement("li"); li.className = "timeline-item"; if (index >= EXP_LIMIT) li.classList.add("hidden-item");
    ...
    if(compList && config.competences) {
        config.competences.forEach((comp, index) => {
            const li = document.createElement("li"); li.className = "comp-card-container"; if (index >= COMP_LIMIT) li.classList.add("hidden-item");
            const dropId = `comp-drop-${index}`; const details = comp.details.map(d => `<li>• ${escapeHTML(d)}</li>`).join('');
            ...
            const headerEl = li.querySelector('.comp-header'); headerEl.addEventListener("click", (e) => { toggleComp(dropId, headerEl); });
    ...
    if(certList && config.certifications) {
        config.certifications.forEach((cert, index) => {
            const li = document.createElement("li"); li.className = "cert-card-container"; if (index >= CERT_LIMIT) li.classList.add("hidden-item");
            const issuer = cert.issuer ? cert.issuer : "Certification"; const viewerId = `cert_view_${index}`; const fullPdfUrl = cert.pdf ? certBaseUrl + cert.pdf : null; let buttonsHtml = '';
            ...
            if (cert.pdf) {
                const actionsDiv = li.querySelector('.cert-actions'); const pdfBtn = document.createElement("button"); pdfBtn.className = "cert-btn pdf-btn"; pdfBtn.title = "Voir le diplôme"; pdfBtn.innerHTML = "📄";
                pdfBtn.addEventListener("click", () => { toggleCertPDF(viewerId, fullPdfUrl); }); actionsDiv.appendChild(pdfBtn);
            }
    ...
    // --- TYPEWRITER / SCROLL / FOOTER ---
    const textEl = document.getElementById("typewriter-area");
    if(textEl && config.profile.typewriterText) { const txt = config.profile.typewriterText; textEl.innerText = ""; let i=0; function type() { if(i<txt.length) { textEl.textContent += txt.charAt(i); i++; setTimeout(type, 50); } } setTimeout(type, 500); }
    const header = document.querySelector('.app-header'); const menuIcon = document.querySelector('.menu-icon'); const navCapsule = document.querySelector('.nav-capsule');
    if (header) { window.addEventListener('scroll', () => { if (window.scrollY > 50) header.classList.add('scrolled'); else { header.classList.remove('scrolled'); header.classList.remove('menu-open'); } }); if (menuIcon) { menuIcon.addEventListener('click', (e) => { e.stopPropagation(); header.classList.toggle('menu-open'); }); } document.addEventListener('click', (e) => { if (header.classList.contains('menu-open') && navCapsule && !navCapsule.contains(e.target)) { header.classList.remove('menu-open'); } }); }
    const updateEl = document.getElementById("last-update");
    if(updateEl && config.profile.githubUser && config.profile.githubRepo) { const repoUrl = `https://api.github.com/repos/${config.profile.githubUser}/${config.profile.githubRepo}`; fetch(repoUrl).then(response => { if (!response.ok) throw new Error("Repo not found"); return response.json(); }).then(data => { const date = new Date(data.pushed_at); const formattedDate = date.toLocaleDateString('fr-FR') + ' ' + date.toLocaleTimeString('fr-FR', {hour: '2-digit', minute:'2-digit'}); updateEl.innerHTML = `Maj : ${formattedDate}`; }).catch(err => { updateEl.innerText = "System Ready"; }); }
    
    document.addEventListener('keydown', (e) => { if (e.key === "Escape") { document.querySelectorAll('.modal-overlay').forEach(m => m.style.display = "none"); document.querySelectorAll('.pdf-container').forEach(el => { el.style.display='none'; el.innerHTML=''; }); document.querySelectorAll('.cert-pdf-viewer').forEach(el => { el.style.display='none'; el.innerHTML=''; }); document.querySelectorAll('.comp-dropdown-menu').forEach(el => el.style.display='none'); document.querySelectorAll('.comp-toggle').forEach(el => el.classList.remove('active')); } if ((e.key === "d" || e.key === "D") && e.target.tagName !== 'INPUT') { document.getElementById("theme-toggle").click(); } });
});

function openProjectModal(proj) { const modal = document.getElementById("project-modal"); const titleEl = document.getElementById("modal-project-title"); const descEl = document.getElementById("modal-project-desc"); const tagsEl = document.getElementById("modal-project-tags"); if(modal && titleEl && descEl && tagsEl) { titleEl.innerText = proj.title; descEl.innerHTML = proj.longDescription ? proj.longDescription : proj.description; tagsEl.innerHTML = ""; if(proj.tags && proj.tags.length > 0) { proj.tags.forEach(tag => { const span = document.createElement("span"); span.className = "project-tag"; span.innerText = tag; tagsEl.appendChild(span); }); } else { tagsEl.innerHTML = "<span style='color:var(--muted); font-size:0.8rem;'>Aucun tag</span>"; } modal.style.display = "flex"; } }
