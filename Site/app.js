    ...
    if(expList && config.experiences) {
        config.experiences.forEach((exp, index) => {
            ...
            const photoHtml = exp.photo
                ? `<img src="assets/${escapeHTML(exp.photo)}" alt="${escapeHTML(exp.company)}" style="width:36px;height:36px;border-radius:6px;object-fit:cover;border:1px solid var(--border,#2a2f45);flex-shrink:0;" onerror="this.style.display='none'">`
                : '';
            li.innerHTML = `
